	...
	public async init(
		...
		return new Promise((resolve) => {
			...
			emitter.once('tdoc.build.init', () => {
				...
				!testing &&
					browser.init({ server: this.getHttpRoot(this.opts) }); //cannot get Mocha/sinon to stub browserSync
			...
			emitter.on('log.message', (context, message, type, prefix) =>
				this.logger(context, message, type, prefix)
			);
		});
	}
}
