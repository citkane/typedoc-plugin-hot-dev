...
describe('Unit testing for typedoc-plugin-hot-dev', function () {
	...
	it('creates and transforms options', function () {
		...
		assert.hasAnyKeys(
			...
			'did not generate root keys'
		);
		assert.equal(
			stripTrailing(this.opts.targetCwdPath),
			stripTrailing(cwd),
			'did not resolve the path for "targetCwd" correctly'
		);
		assert.equal(
			stripTrailing(this.opts.sourceMediaPath),
			stripTrailing(sourceMediaPath),
			'did not resolve the path for "sourceMediaPath" correctly'
		);
		assert.equal(
			stripTrailing(this.opts.sourceDistPath),
			stripTrailing(sourceDistPath),
			'did not resolve the path for "sourcDistPath" correctly'
		);
	});
	it('explicitly fetches the root http path', function () {
		assert.equal(
			this.hot.getHttpRoot({ targetCwdPath: process.cwd() }),
...
describe('Functional testing for typedoc-plugin-hot-dev', function () {
	...
	it(`spawns a tsc process that compiles to the "${sourceDistPath}"`, function (done) {
		this.timeout(10000);
		this.tsc = this.hot.spawnTscWatch(this.emitter, new AbortController(), {
			sourceDistPath,
			tsc: {},
			tdoc: {},
		});
		...
		assert.exists(
			...
			'spawn did not return a controller'
		);
		assert.isObject(
			this.tsc.process,
			'spawn did not return a process object'
		);
		assert.exists(
			this.tsc.process.kill,
			'spawn did not return a process kill'
		);

		this.emitter.on('tsc.compile.done', () => {
			assert.isTrue(
				fs.existsSync(sourceDistPath),
				`tsc did not create the "${sourceDistPath}" folder`
			);
			assert.isTrue(
				fs.existsSync(path.join(sourceDistPath, 'index.js')),
				'tsc did not compile "index.js" from "index.ts"'
			);
			assert.isTrue(
				fs.existsSync(path.join(sourceDistPath, 'index.d.ts')),
				'tsc did not compile "index.ts" with a "index.d.ts"'
			);
			done();
		});
	});
	it(`watches the "${cwd}/src" folder and compiles on change`, async function () {
		...
		assert.isTrue(
			hasWatched,
			'the tsc compiler is not watching the src folder'
		);
		this.tsc.controller.abort();
	...
	it(`spawns a typedoc process that builds docs to the "${targetDocDir}" folder`, async function () {
		this.timeout(30000);
		const allOps = this.hot.parseOptions(
			{ overrideHot },
			new Application()
		);
		...
		const hasBuiltDocs = await waitForFile(
			path.join(targetDocPath, 'index.html'),
			this.timeout()
		);
...
describe('End to End test for typedoc-plugin-hot-dev', function () {
	...
	it('starts a tsc compiler in watch mode and runs the initial doc build', async function () {
		...
		emitter.on('log.message', (context, message) => {
			if (context === 'npm') {
				...
				message.indexOf('Missing script: "foo"') > -1 &&
					(this.npmTestFailed = true);
			}
		});
		({
			tsc: this.tsc,
	...
	it('updates asset files on change', function (done) {
		...
		setTimeout(async () => {
			...
			assert.isTrue(
				wasUpdated,
				'update was not triggered on asset change'
			);
			done();
		}, 100);
	});
